# Lesson 1: Find a File in a Much Bigger Drive

**Time:** ~15-20 minutes · **Level:** 17 — Welcome to HQ · **Track:** 16 —
Getting Oriented at HQ
**Task key:** `office-drive` · **Status:** not built yet. This is the
promotion into Office Administrator — the environment refreshes here: new
building, new title, new team.
**Prerequisite:** Act V complete (either path, or skipped) — the
environment refreshes here: new emails, a new building (Harborside's HQ
office, not the cafe), a new wallpaper.

## We will learn...
- find one specific file in a shared drive that's much bigger and more
  deeply nested than the one from Level 3 — folders inside folders, several
  similarly-named files — using search, not just browsing.

## Before class (teacher prep) — once built
- [ ] Simulator: an HQ-scale Hdrive with several folders, sub-folders
      inside those, and 3-4 similarly-named files (different dates or
      versions) so the student has to read carefully, not just guess.
- [ ] No Google Classroom post needed yet — this is optional, supplementary
      practice bundled into the Level 17 wrap-up assignment
      (`real-practice-assignment.md`), offered once this lesson is done.
      Never a gate — a student who skips it still moves on.

## Simulator practice (~15-20 min) — once built
1. Open **Hdrive** at HQ — notice it's much bigger than Level 3's shared
   drive.
2. Use the search bar rather than clicking through every folder — the same
   "search instead of browsing blind" instinct from Level 3's `files`
   task, now genuinely necessary instead of optional.
3. Read the similarly-named results carefully — pick the one that actually
   matches what was asked (right date, right version), not just the first
   result.
4. Share it with the right access level, the same judgment call from
   Level 3.

## Confidence check-in
- "I could find the right file in a big shared drive without getting lost
  or grabbing the wrong version." 🙂 😐 🙁
- "The thing that would make me most likely to grab the wrong file at a
  real job is: ___________"

## Notes for next time
- This is the same underlying skill as Level 3's `files`, scaled up — a
  student who was comfortable there should recognize it, just harder. If a
  student grabs a wrong-version file, that's the actual teaching moment,
  not a failure to route around.

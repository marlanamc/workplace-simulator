# Lesson 1: How This Computer Works

**Time:** ~5 minutes · **Level:** 0 — How this works · **Track:** Orientation
**Task key:** `tour` · **Status:** built and playable today
**Prerequisite:** Sign in. Track 0 (device, mouse, typing) if they have never
used a Chromebook. This level is for everyone, including people who already
know computers — it is how *this* practice space works, not how a computer
works.

## We will learn...
*(read aloud / post on board — plain language, one idea)*
- this is practice, you cannot break it; Help is the question mark; Next is
  the blue button; the target on the bottom bar is your list for this shift.

## Before class (teacher prep)
- [ ] Simulator: nothing to set up. A new sign-in lands on the desktop
      briefing "This is a practice computer." Click **Open the Web Browser**.
- [ ] People who already finished later levels are not pulled back here.
      They can still open Level 0 from the levels list if they want the boost.

## Simulator practice (~5 min)
1. Sign in with name, PIN, and class code.
2. Read the desktop briefing. Click **Open the Web Browser**.
3. Read the first card. Click **Show me around**.
4. Tap all three facts (nothing is real; the white button is next; Help and
   your list). Click **I have these**.
5. Click the **?** at the top right. Read Help. Click **Got it**.
6. Click **I'm ready for the job**.
7. On the done screen, the blue button is **Open Mail** — that is Level 1.

If they skip Help, a short toast tells them to click the ? first. That is
the whole point of this step.

## Confidence check-in
The app asks this on the done screen. Worth a glance:
- "Do you know where to look if you get stuck?"

## Notes for the teacher
- This is not Track 0. Track 0 is the physical Chromebook (power, mouse,
  Classroom). Level 0 is inside the simulator, for every student.
- Keep it short. If someone flies through, let them go to Level 1 the same
  sitting. The boost is the three places to look, not extra clicking.

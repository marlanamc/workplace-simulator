# Lesson 1: Handle a Meeting Invite

**Time:** ~5 minutes · **Level:** 3 — Shift Lead · **Track:** 4 — Growing at Work
**Task key:** `calendar` · **Status:** built — the first task of Level 3.
Note: from this task on, the desktop's "Do this next" card no longer opens
the exact tab for you (it just says "Open Browser") — finding the right
bookmark is deliberately part of the task now that a student has some
practice under their belt.
**Prerequisite:** Level 2 complete (this is the promotion the earlier
levels were building toward — the environment refreshes here: new emails,
new schedule, a new week at Harborside Cafe)

## We will learn...
- accept or decline a meeting invite, and speak up when it collides with a
  shift you're already working — the same "notice it, say something" move
  from `schedule`, in a new tool.

## Before class (teacher prep)
- [ ] Simulator: nothing to set up — the Calendar tab always has the same
      invite (a weekly huddle on a day the student isn't scheduled).
- [ ] No Google Classroom post needed yet — this is optional, supplementary
      practice bundled into the Level 3 wrap-up assignment
      (`real-practice-assignment.md`), offered after Track 4 is done. Never
      a gate — a student who skips it still moves on.

## Simulator practice (~5 min)
1. Click **Open Browser** on the desktop — it won't jump straight to
   Calendar anymore. Find the **Calendar** bookmark yourself, in the tab
   strip or the bookmarks bar.
2. Find the meeting invite and read what it's asking (who, when, why).
3. Check it against your schedule — does it overlap a shift?
4. If it does, don't just accept it — click **Propose a different time**
   and write a short reply, the same way you asked for a shift swap in
   Level 2's Lesson 1.
5. If it doesn't conflict, accept it.

## Confidence check-in
- "I could handle a meeting invite that conflicts with my shift." 🙂 😐 🙁
- "The part that felt hardest was: ___________"

## Notes for next time
- This is deliberately the *same skill* as Level 2's Lesson 1 (`schedule`)
  in a new tool — if a student breezes through this, that's the point
  landing: new logo, same judgment call.

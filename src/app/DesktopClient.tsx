...
export default function DesktopClient({
  ...
  const apps = useMemo(
    () =>
      APP_DEFS.map((a) => ({
        ...a,
        state: (completedTaskKeys.includes(a.key) ? "done" : a.state) as AppState,
      })),
    [completedTaskKeys]
  );

  const focusApp = apps[0];
  const mailDone = completedTaskKeys.includes("mail");
  const activeApp = openApp ? apps.find((a) => a.key === openApp) : null;
  const doneCount = apps.filter((a) => a.state === "done").length;

  const filteredApps = useMemo(() => {
    const q = query.trim().toLowerCase();
    if (!q) return apps;
    return apps.filter((a) => appCopy[a.key].name.toLowerCase().includes(q));
  }, [query, appCopy, apps]);
          ...
          <div className="flex items-center gap-3">
            <div className="h-1.5 flex-1 overflow-hidden rounded-full bg-[var(--surface-muted)]">
              <div
                ...
                style={{ width: `${Math.round((doneCount / apps.length) * 100)}%` }}
              />
            </div>
            <span className="whitespace-nowrap text-[13px] font-medium text-[var(--text-tertiary)]">
              {doneCount}/{apps.length} {c.progressWord}
            </span>
          </div>
        </div>
      </div>

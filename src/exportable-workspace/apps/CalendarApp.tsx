  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-white text-[14px] text-[#3c4043]" style={{ fontFamily: "Roboto, Arial, sans-serif" }}>
      <div className="flex items-center gap-3 px-3 py-2">
        <CalendarMark />
        <span className="w-[190px] shrink-0 text-[22px] font-normal text-[#5f6368]">{c.calendar}</span>
      ...
      <div className="relative flex min-h-0 flex-1">
        <aside className="flex w-[220px] shrink-0 flex-col gap-4 px-3 pt-1">
          <button onClick={() => host.onNudge?.(c.createLater)} className="flex h-14 items-center gap-3 rounded-2xl bg-white px-4 text-[14px] font-medium shadow-sm ring-1 ring-[#dadce0]">
        ...
        <main className="flex min-w-0 flex-1 flex-col">
          <div className="flex flex-wrap items-center gap-2 px-2 py-1">
            <button onClick={() => host.onNudge?.(c.todayOnly)} className="inline-flex h-9 items-center rounded-full border border-[#dadce0] px-4 text-[14px] font-medium hover:bg-[#f8f9fa]">
              {c.today}
            </button>
            ...
            <div className="flex items-center rounded-lg border border-[#dadce0] text-[13px]">
              {[c.day, c.week, c.monthView].map((label, index) => (
                <button key={label} className={`h-8 px-3 ${index === 2 ? "bg-[#e8f0fe] font-medium text-[#1967d2]" : "hover:bg-[#f1f3f4]"}`}>
                  {label}
                </button>
              ))}
            </div>
          ...
          <div className="grid min-h-0 flex-1 grid-cols-7 grid-rows-6">
            {monthCells.map((cell, index) => {
              const events = cell.other ? [] : calendarEvents.filter((event) => event.day === cell.day);
              return (
                <div key={index} className="flex flex-col gap-0.5 border-b border-r border-[#dadce0] p-1">
                  <span className={`self-center flex h-6 w-6 items-center justify-center rounded-full text-[12px] ${cell.day === 21 && !cell.other ? "bg-[#1a73e8] font-medium text-white" : cell.other ? "text-[#bdc1c6]" : "text-[#3c4043]"}`}>
                    {cell.day}
                  </span>
                  {events.map((event) => (
                    <button key={event.id} onClick={() => setSelected(event)} className="truncate rounded px-1 py-0.5 text-left text-[11px] font-medium text-white" style={{ background: event.color }}>
        ...
        {selected && <EventDialog event={selected} closeLabel={c.close} onClose={() => setSelected(null)} onComplete={() => host.onComplete?.({ appKey: "calendar", action: "open-event", label: selected.title })} />}
      </div>
    </div>
  );
}

function EventDialog({ event, closeLabel, onClose, onComplete }: { event: CalendarEvent; closeLabel: string; onClose: () => void; onComplete: () => void }) {
  return (
      ...
      <div className="w-[min(100%-2rem,420px)] overflow-hidden rounded-3xl bg-white shadow-[0_4px_8px_3px_rgba(60,64,67,.15)]">
        ...
        <div className="px-6 pb-5 pt-4">
          <div className="mb-3 flex items-start justify-between gap-3">
            ...
            <button onClick={onClose} aria-label={closeLabel} className="flex h-9 w-9 items-center justify-center rounded-full text-[#5f6368] hover:bg-[#f1f3f4]">
              x
            </button>
          </div>
          <p>{`Aug ${event.day}, 2026 - ${event.time}`}</p>
          {event.description && <p className="mt-3 leading-relaxed">{event.description}</p>}
          <button onClick={onComplete} className="mt-4 rounded-full bg-[#1a73e8] px-4 py-2 text-[13px] font-medium text-white">
            Mark reviewed
          </button>
        </div>
      </div>
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-7 text-center text-[10px] text-[#70757a]">
        {labels.map((day) => <div key={day}>{day[0]}</div>)}
      </div>
      <div className="grid grid-cols-7 text-center text-[11px]">
        {monthCells.slice(0, 42).map((cell, index) => (
          <span key={index} className={`py-1 ${cell.other ? "text-[#bdc1c6]" : "text-[#3c4043]"}`}>{cell.day}</span>
        ))}
      </div>
    </div>
  );
...
function CalendarMark() {
  return (
    <span className="flex h-8 w-8 flex-col overflow-hidden rounded-[6px] border border-[#dadce0] bg-white shadow-sm">
      ...
      <span className="flex flex-1 items-center justify-center text-[13px] font-medium leading-none text-[#3c4043]">31</span>
    </span>
  );
}

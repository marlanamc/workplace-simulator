...
export function PdfReaderApp({ host }: WorkspaceAppProps) {
  const [activeId, setActiveId] = useState(pdfDocuments[0]?.id ?? "");
  const [zoom, setZoom] = useState(100);
  const active = pdfDocuments.find((doc) => doc.id === activeId) ?? pdfDocuments[0];
  const scale = zoom / 100;
  const notAvailable = () => host.onNudge?.("This simulated reader only supports viewing fixture documents.");
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="flex w-[260px] shrink-0 flex-col border-r border-[#dadce0] bg-white">
          ...
          <div className="flex-1 overflow-y-auto">
            {pdfDocuments.map((doc) => (
              ...
              >
                <span className="shrink-0 rounded bg-[#d93025] px-1.5 py-0.5 text-[10px] font-bold text-white">PDF</span>
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-[14px] text-[#202124]">{doc.name}</span>
                  <span className="block text-[12px] text-[#5f6368]">{doc.size} - {doc.date}</span>
                </span>
              </button>
            ))}
          </div>
        ...
        <main className="flex min-w-0 flex-1 flex-col">
          <div className="flex items-center justify-center gap-1 border-b border-[#3a3d40] bg-[#323639] px-3 py-1.5">
            ...
            <button onClick={() => setZoom((value) => Math.max(60, value - 10))} className="flex h-7 w-7 items-center justify-center rounded text-white/85 hover:bg-white/10" aria-label="Zoom out">
              <Minus size={15} />
            </button>
            <span className="w-11 text-center text-[12px] text-white/85">{zoom}%</span>
            <button onClick={() => setZoom((value) => Math.min(150, value + 10))} className="flex h-7 w-7 items-center justify-center rounded text-white/85 hover:bg-white/10" aria-label="Zoom in">
              <Plus size={15} />
            </button>
            <span className="mx-2 h-4 w-px bg-white/20" />
            <span className="text-[12px] text-white/70">Page 1 / 1</span>
            <span className="mx-2 h-4 w-px bg-white/20" />
            <button onClick={notAvailable} className="flex h-7 w-7 items-center justify-center rounded text-white/85 hover:bg-white/10" aria-label="Print">
              <Printer size={15} />
            </button>
            <button onClick={notAvailable} className="flex h-7 w-7 items-center justify-center rounded text-white/85 hover:bg-white/10" aria-label="Download">
              <Download size={15} />
            </button>
          </div>
            ...
            <div className="absolute inset-0 overflow-auto">
              <div className="flex justify-center" style={{ padding: 28, minWidth: "min-content" }}>
                <div className="shrink-0" style={{ width: `${letter.widthIn * scale}in`, height: `${letter.heightIn * scale}in` }}>
                  ...
                  >
                    <PdfPage>
                      {active?.kind === "report" ? <ReportPage doc={active} /> : active ? <PayStubPage doc={active} /> : null}
                    </PdfPage>
                  </div>
                </div>
              </div>
...
function PdfPage({ children }: { children: ReactNode }) {
  return (
    <div className="relative box-border h-full w-full" style={{ padding: "1in 1in 1.15in", fontFamily: '"Times New Roman", Times, Georgia, serif', fontSize: "12pt", lineHeight: 1.35, color: "#1a1a1a" }}>
      {children}
      <div className="absolute flex items-center justify-between border-t border-[#1a1a1a]/30 text-[9pt] text-[#444]" style={{ left: "1in", right: "1in", bottom: "0.55in", paddingTop: "0.2in" }}>
...
function ReportPage({ doc }: { doc: Extract<PdfDocument, { kind: "report" }> }) {
  return (
    <>
      ...
      <p className="mt-[28pt] text-[12pt] leading-relaxed">Prepared by<br /><span className="italic">{doc.signedBy}</span></p>
    </>
  );
}
        ...
        <tbody>
          {doc.earnings.map((earning) => (
            <tr key={earning.label}>
              ...
              <td className="border border-[#1a1a1a] px-[0.7em] py-[0.3em] text-right tabular-nums">{earning.amount}</td>
            </tr>
          ))}
          <tr>
            <td className="border border-[#1a1a1a] px-[0.7em] py-[0.3em] font-bold" colSpan={2}>Gross pay</td>
        ...
        <tbody>
          {doc.deductions.map((deduction) => (
            <tr key={deduction.label}>
              ...
              <td className="border border-[#1a1a1a] px-[0.7em] py-[0.3em] text-right tabular-nums">{deduction.amount}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <table className="w-full border-collapse text-[1em]">
        <tbody>
          <tr>
            ...
            <td className="border-[2px] border-[#1a1a1a] px-[0.7em] py-[0.45em] text-right text-[1.25em] font-bold tabular-nums">{doc.netPay}</td>
          </tr>
        </tbody>
      </table>
    </>

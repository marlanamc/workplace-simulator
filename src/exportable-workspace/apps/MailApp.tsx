      ...
      <div className="flex min-h-0 flex-1">
            ...
            {mailMessages.map((message) => (
              ...
              >
                <span className="min-w-0">
                  ...
                  <span className="block truncate text-[12px] font-normal text-[#5f6368]">{message.subject}</span>
                </span>
        ...
        <div className="min-w-0 flex-1">
          {composing ? (
            ...
            <MessageView message={selected} lang={host.lang} onReply={() => setComposing(true)} replyLabel={c.reply} />
          ) : null}
        </div>
      </div>
    </div>
  ...
  return (
    <article className="mx-auto max-w-[760px] px-8 py-6">
      ...
      <div className="mt-5 flex items-start gap-3 border-b border-[#eef0f3] pb-5">
        ...
        <button onClick={onReply} className="rounded-full border border-[#dadce0] px-4 py-2 text-[13px] font-medium text-[#1a73e8] hover:bg-[#f2f6fc]">
          {replyLabel}
        </button>
      </div>
    ...
    <div className="mx-auto mt-8 w-[min(100%-2rem,680px)] overflow-hidden rounded-2xl border border-[#dadce0] bg-white shadow-[0_4px_14px_rgba(60,64,67,.2)]">
      ...
      <div className="px-5">
        ...
        <div className="flex items-center gap-2 pb-4">
          <button onClick={onSend} className="inline-flex min-h-[36px] items-center rounded-full bg-[#0b57d0] px-6 text-[14px] font-medium text-white">
            {sendLabel}
          </button>

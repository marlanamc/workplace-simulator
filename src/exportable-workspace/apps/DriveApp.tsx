    ...
    <div className="flex h-full min-h-0 flex-col bg-white text-[14px] text-[#1f1f1f]" style={{ fontFamily: "Roboto, Arial, sans-serif" }}>
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="flex w-[220px] shrink-0 flex-col gap-0.5 px-3 pt-1">
          <button onClick={() => host.onNudge?.(c.notAvailable)} className="mb-3 flex h-14 items-center gap-3 rounded-2xl bg-white px-4 text-[14px] font-medium shadow-sm ring-1 ring-[#dadce0]">
            <Plus size={20} />
            {c.new}
          </button>
          <NavButton active={!folder} icon={<Home size={18} />} label={c.home} onClick={() => setFolder(null)} />
          <NavButton active={false} icon={<Folder size={18} />} label={c.myDrive} onClick={() => host.onNudge?.(c.useShared)} />
          <NavButton active={Boolean(folder)} icon={<Users size={18} />} label={c.shared} onClick={() => setFolder(null)} />
        </aside>
        <main className="relative min-w-0 flex-1 overflow-y-auto px-4 pb-6 pt-2">
          ...
          <div className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
            {folders.map((name) => (
              <button key={name} onClick={() => setFolder(name)} className="flex items-center gap-3 rounded-xl bg-[#f0f4f9] px-4 py-3 text-left hover:bg-[#e8eaed]">
          ...
          {selected && <ShareDialog file={selected} labels={c} onCancel={() => setSelected(null)} onShare={share} />}
        </main>
      </div>
    </div>
  );
...
function FileTable({ files, onSelect, emptyLabel }: { files: DriveFile[]; onSelect: (file: DriveFile) => void; emptyLabel: string }) {
  return (
    <div>
      ...
      {files.map((file) => (
        <button key={file.id} onClick={() => onSelect(file)} className="grid w-full grid-cols-[1fr_140px_88px] items-center gap-2 rounded-xl px-3 py-2.5 text-left hover:bg-[#f1f3f4]">
    ...
    <div className="absolute inset-0 z-10 flex items-start justify-center bg-black/32 pt-16">
      <div className="w-[min(100%-2rem,420px)] rounded-3xl bg-white p-6 shadow-[0_4px_8px_3px_rgba(60,64,67,.15)]">
        ...
        <div className="flex flex-col gap-2">
          <button onClick={() => onShare("view")} className="flex min-h-[44px] items-center justify-between rounded-lg border border-[#dadce0] px-3 text-left text-[14px] hover:bg-[#f8f9fa]">
            <span>{labels.canView}</span>
            <span className="text-[12px] text-[#5f6368]">Viewer</span>
          </button>
          <button onClick={() => onShare("edit")} className="flex min-h-[44px] items-center justify-between rounded-lg border border-[#dadce0] px-3 text-left text-[14px] hover:bg-[#f8f9fa]">
            <span>{labels.canEdit}</span>
            <span className="text-[12px] text-[#5f6368]">Editor</span>
          </button>
        </div>
        <div className="mt-6 flex justify-end">
          <button onClick={onCancel} className="h-10 rounded-full px-4 text-[14px] font-medium text-[#0b57d0] hover:bg-[#f2f6fc]">
            {labels.cancel}
          </button>
        </div>
      </div>
...
function FileType({ type }: { type: DriveFile["type"] }) {
  ...
  return (
    <span className="flex h-6 w-8 shrink-0 items-center justify-center rounded-[2px] text-[8px] font-bold text-white" style={{ background: color }}>
      {label}
    </span>
  );
}
...
function NavButton({ active, icon, label, onClick }: { active: boolean; icon: ReactNode; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`flex h-10 items-center gap-3 rounded-full px-4 text-[14px] ${active ? "bg-[#c2e7ff] font-medium text-[#041e49]" : "text-[#444746] hover:bg-[#e8eaed]"}`}>
      {icon}
      {label}
    </button>
  );

...
export function PortalApp({ host }: WorkspaceAppProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[#f8fafd] text-[15px] text-[#202124]">
        ...
        {portalSections.map((section) => (
          <button
            ...
            className={`rounded-t-lg px-4 py-2.5 text-[14px] font-medium ${
              section.key === active?.key ? "border-b-2 border-[#8430ce] text-[#8430ce]" : "text-[#5f6368] hover:text-[#202124]"
            }`}
          >
            {section.label}
          </button>

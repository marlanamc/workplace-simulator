    ...
    <div className="flex min-h-[620px] flex-1 flex-col overflow-hidden rounded-lg border border-[#c7c9cc] bg-[#dee1e6] text-[#202124] shadow-sm">
      ...
      <div className="flex items-center gap-2 bg-white px-2 py-1.5">
        ...
        <div className="flex h-9 flex-1 items-center gap-2 rounded-full bg-[#e9eef6] px-3 text-[14px]">
          {active?.url === "newtab" ? <Search size={16} className="text-[#5f6368]" /> : <LockIcon />}
          <span className={`min-w-0 flex-1 truncate ${active?.url === "newtab" ? "text-[#5f6368]" : "text-[#202124]"}`}>
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 rounded-md px-2 py-[5px] text-[13px] ${
                active?.key === app.key && active.url !== "newtab" ? "bg-[#e8eaed] text-[#202124]" : "text-[#3c4043] hover:bg-[#f1f3f4]"
              }`}
            >
              <span className="flex h-4 w-4 items-center justify-center rounded-[3px] text-white" style={{ background: app.color }}>
...
function ShellTab({
  ...
  const Icon = tab.icon;
  return (
    <div className={`group relative flex h-[34px] min-w-[92px] max-w-[220px] flex-1 items-center ${active ? "z-10" : "z-0"}`}>
      <span className={`absolute ${active ? "inset-0 rounded-t-[10px] bg-white" : "inset-[3px_2px] rounded-lg group-hover:bg-black/[0.08]"}`} />
      <button onClick={onSelect} title={tab.label} className="relative z-10 flex h-full min-w-0 flex-1 items-center gap-2 pl-3 pr-1">
        <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-[3px] text-white" style={{ background: tab.color }}>
          <Icon size={11} strokeWidth={2.5} />
        </span>
        <span className={`min-w-0 flex-1 truncate text-left text-[13px] ${active ? "text-[#202124]" : "text-[#474747]"}`}>
          {tab.label}
        </span>
      </button>
      {canClose && (
...
function ToolbarButton({ label, children }: { label: string; children: string }) {
  return (
    <button className="flex h-8 w-8 items-center justify-center rounded-full text-[20px] text-[#5f6368] hover:bg-black/[0.06]" aria-label={label}>
      {children}
    </button>
  );
}
...
function NewTab({ apps, onOpen }: { apps: WorkspaceAppDefinition[]; onOpen: (app: WorkspaceAppDefinition) => void }) {
  return (
    <div className="flex h-full flex-col items-center bg-white pt-[12vh]">
      ...
      <div className="mt-8 grid grid-cols-4 gap-4">
        {apps.map((app) => {
          const Icon = app.icon;
          return (
            <button key={app.key} onClick={() => onOpen(app)} className="flex w-20 flex-col items-center gap-2 rounded-lg p-2 text-[12px] text-[#3c4043] hover:bg-[#f1f3f4]">
              <span className="flex h-10 w-10 items-center justify-center rounded-xl text-white" style={{ background: app.color }}>
                <Icon size={20} />
              </span>
              {app.label}
            </button>

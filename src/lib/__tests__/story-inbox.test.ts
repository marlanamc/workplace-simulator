...
const FAR_ALONG: TaskKey[] = LEVELS.flatMap((l) => taskKeysForLevel(l, null)).slice(
  0,
  LEVELS.flatMap((l) => taskKeysForLevel(l, null)).indexOf("calendar") + 1,
);
...
describe("storyMailsUpTo", () => {
  ...
  it("every story mail's unlockAfter task exists in the curriculum order", () => {
    const order = LEVELS.flatMap((l) => taskKeysForLevel(l, null));

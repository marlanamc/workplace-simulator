...
async function preset(page:Page, name:RegExp) {
 ...
 // The early-act arrival opens Mail directly into the compose view.
...
for (const lang of ['en', 'es'] as const) {
  test(`short absence reply recovers after sickness-only rejection (${lang})`, async ({ page }) => {
    await signup(page, lang);
    await preset(page, /The Sick Call/);
    const card = page.locator('[data-job-card]');
    const body = page.locator('textarea').first();
    const send = page.getByRole('button', { name: /^(Send|Enviar)$/ });
    const incomplete = lang === 'en' ? 'I feel sick today.' : 'Estoy enferma hoy.';
    await body.fill(incomplete);
    await send.click();
    await expect(card).toContainText(lang === 'en' ? "cannot work today's shift" : 'no puedes trabajar tu turno de hoy');
    await expect(body).toHaveValue(incomplete);
    await body.fill(lang === 'en' ? 'I can’t work today.' : 'No puedo trabajar hoy.');
    await send.click();
    await expect(body).toHaveCount(0);
    await expect(page.getByText(DONE_COPY['call-out-sick'][lang].kicker, { exact: true }).first()).toBeVisible({ timeout: 20_000 });
